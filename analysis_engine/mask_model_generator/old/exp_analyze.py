...
def rename_keys(denested):
    denested.pop('WebGL.Data')
    denested['webGL.Renderer'] = denested.pop('WebGL.Renderer')
    denested['webGL.Vendor'] = denested.pop('WebGL.Vendor')
    denested['webGL.Data Hash'] = denested.pop('WebGL.hash of data')
    denested.pop('canvas')
    denested['canvas fingerprint'] = denested.pop('hash of canvas data')
...
purported_effect = {
    ...
    ('firefox', 'tor'): {'h.User-Agent', 'h.Accept', 'h.Accept-Encoding', 'h.Accept-Language', 'language', 'javascript fonts', 'canvas fingerprint', 
            ...
            'screen.AvailHeight', 'screen.AvailLeft', 'screen.AvailTop', 'screen.AvailWidth', 
            'screen.Depth', 'screen.Height', 'screen.Left', 'screen.Pixel Ratio', 'screen.Top', 'screen.Width', 'color depth', 
            'webGL.Vendor', 'webGL.Renderer', 'webGL.Data Hash', 'webGL.Data'},
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print('Example: python exp_analyze.py exp_data/exp08')
        sys.exit(0)
    folder = sys.argv[1]
    oses = ["ubuntu/xenial64", "debian/jessie64", "ubuntu/trusty64", "macos"]
    configs =   { 
            "jessie64": ["1","6"],
            "trusty64":["2","4"], # 4
            "xenial64": ["3","5"], # 5
